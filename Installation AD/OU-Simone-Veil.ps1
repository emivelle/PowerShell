# Définir le chemin racine pour l'OU Simone Veil
$RootOU = "OU=Simone Veil,DC=GRESIVAUDAN,DC=LOCAL"

# Création de l'OU Simone Veil
if (-not (Get-ADOrganizationalUnit -Filter "Name -eq 'Simone Veil'" -SearchBase "DC=GRESIVAUDAN,DC=LOCAL" -ErrorAction SilentlyContinue)) {
    New-ADOrganizationalUnit -Name "Simone Veil" -Path "DC=GRESIVAUDAN,DC=LOCAL"
    Write-Host "OU 'Simone Veil' créée."
} else {
    Write-Host "OU 'Simone Veil' existe déjà."
}
...
for ($i = 1; $i -le 17; $i++) {
    ...
    if (-not (Get-ADOrganizationalUnit -Filter "Name -eq '$ClassOUName'" -SearchBase $RootOU -ErrorAction SilentlyContinue)) {
        ...
        Write-Host "OU '$ClassOUName' créée dans 'Simone Veil'."
    } else {
