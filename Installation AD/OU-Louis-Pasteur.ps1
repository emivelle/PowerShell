# Définir le chemin racine pour l'OU Louis Pasteur
$RootOU = "OU=Louis Pasteur,DC=GRESIVAUDAN,DC=LOCAL"

# Création de l'OU Louis Pasteur
if (-not (Get-ADOrganizationalUnit -Filter "Name -eq 'Louis Pasteur'" -SearchBase "DC=GRESIVAUDAN,DC=LOCAL" -ErrorAction SilentlyContinue)) {
    New-ADOrganizationalUnit -Name "Louis Pasteur" -Path "DC=GRESIVAUDAN,DC=LOCAL"
    Write-Host "OU 'Louis Pasteur' créée."
} else {
    Write-Host "OU 'Louis Pasteur' existe déjà."
}
...
for ($i = 1; $i -le 15; $i++) {
    ...
    if (-not (Get-ADOrganizationalUnit -Filter "Name -eq '$ClassOUName'" -SearchBase $RootOU -ErrorAction SilentlyContinue)) {
        ...
        Write-Host "OU '$ClassOUName' créée dans 'Louis Pasteur'."
    } else {
